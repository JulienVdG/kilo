...
ifneq ($(REGISTRY),index.docker.io)
    REGISTRY_REFIX := $(REGISTRY)/
endif
MANIFEST_IMAGE := $(REGISTRY_REFIX)$(IMAGE)
...
container-latest: .container-$(ARCH)-$(VERSION)
	@docker tag $(IMAGE):$(ARCH)-$(VERSION) $(REGISTRY_REFIX)$(IMAGE):$(ARCH)-latest
...
.manifest-$(VERSION): Dockerfile $(addprefix push-, $(ALL_ARCH))
	@docker manifest create --amend $(MANIFEST_IMAGE):$(VERSION) $(addsuffix -$(VERSION), $(addprefix $(MANIFEST_IMAGE):, $(ALL_ARCH)))
...
manifest-latest: Dockerfile $(addprefix push-latest-, $(ALL_ARCH))
	@docker manifest rm $(MANIFEST_IMAGE):latest || echo no old manifest
	@docker manifest create --amend $(MANIFEST_IMAGE):latest $(addsuffix -latest, $(addprefix $(MANIFEST_IMAGE):, $(ALL_ARCH)))
	@$(MAKE) --no-print-directory manifest-annotate-latest
	@docker manifest push $(MANIFEST_IMAGE):latest
...
manifest-annotate-%:
	...
	for a in $(ALL_ARCH); do \
		...
		if [ "$$j" -eq "$$i" ] && [ -n "$$da" ]; then \
		    annotate="docker manifest annotate $(MANIFEST_IMAGE):$* $(MANIFEST_IMAGE):$$a-$* --os linux --arch"; \
		    k=0; for ea in $$da; do \
